...
if __name__ == "__main__":
    ...
    Xunif, Iunif = kh.select_design(20)

    kh2 = KernelHerding(
        kernel=kernel,
        initial_design=Xunif,
        candidate_set_size=2 ** 12,
        distribution=ot.ComposedDistribution([ot.Uniform(0.0, 1.0)] * dimension),
    )
    Xunif2, Iunif2 = kh2.select_design(10)

    normal = ot.Normal(2)
    normal2 = ot.Normal([1.0] * 2, [0.2] * 2, ot.IdentityMatrix(2))
    mixture = ot.Mixture([normal, normal2])
    sobol = ot.LowDiscrepancyExperiment(ot.SobolSequence(), mixture, 2 ** 10, True)
    candidate_set = sobol.generate()

    kh3 = KernelHerding(
        kernel=kernel,
        candidate_set=candidate_set,
    )
    Xmixture, Imixture = kh3.select_design(100)

    kh4 = KernelHerding(
        kernel=kernel,
        distribution=normal,
    )
    Xnormal, Inormal = kh4.select_design(100)
